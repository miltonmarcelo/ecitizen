...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [overviewData, usersData, categoriesData] = await Promise.all([
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setError(err.message || "Unable to refresh the admin dashboard.");
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const stats = useMemo(
    () => [
      { title: "Total Users", value: counts.users, icon: Users, color: "text-primary bg-primary/10" },
      {
        title: "Active Users",
      ...
      { title: "Total Issues", value: counts.issues, icon: FileText, color: "text-primary bg-primary/10" },
    ],
    [counts.categories, counts.issues, counts.users, users]
  );

  const quickAccess = [
    { title: "Manage Users", description: "View, edit, and manage all user accounts", icon: Users, url: "/admin/users" },
    { title: "Manage Staff", description: "View and manage staff records", icon: UserCheck, url: "/admin/staff" },
    { title: "Manage Categories", description: "Add, edit, and manage issue categories", icon: FolderOpen, url: "/admin/categories" },
    { title: "Browse Database", description: "Explore all database tables", icon: FileText, url: "/admin/database" },
  ];
  ...
  return (
    <AdminLayout pageTitle="Admin Dashboard">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-heading font-semibold text-foreground mb-1">Overview</h2>
            ...
            <Card key={stat.title} className="shadow-sm">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-heading font-bold text-card-foreground leading-none mt-0.5">{stat.value}</p>
                </div>
              </CardContent>
            </Card>
          );
      ...
      <div>
        <h2 className="text-lg font-heading font-semibold text-foreground mb-1">Quick Access</h2>
